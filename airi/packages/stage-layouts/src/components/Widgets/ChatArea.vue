  ...
  <div h="<md:full" flex gap-2 class="ph-no-capture">
    ...
    >
      ...
      >
        ...
        <!-- Mic button: left-click toggles transcription. Gear button opens hearing settings. -->
        <div class="flex items-center">
          <button
            :class="[
              'h-8 w-8 flex items-center justify-center rounded-md outline-none',
              'transition-all duration-200 active:scale-95',
              isListening ? 'text-primary-500 dark:text-primary-400' : 'text-neutral-500 dark:text-neutral-400',
            ]"
            :title="isListening ? 'Stop transcription' : 'Start transcription'"
            @click="() => isListening ? stopStreamingTranscription() : startStreamingTranscription()"
          >
            <Transition name="fade" mode="out-in">
              <IndicatorMicVolume v-if="isListening" class="h-5 w-5" />
              <div v-else-if="enabled" class="i-ph:microphone h-5 w-5" />
          ...
          <PopoverRoot v-model:open="hearingPopoverOpen">
            <PopoverTrigger as-child>
              <button
                class="h-5 w-5 flex items-center justify-center rounded outline-none text-neutral-400 dark:text-neutral-500 hover:text-neutral-600 dark:hover:text-neutral-300 transition-colors"
                :title="t('settings.pages.modules.hearing.title')"
              >
                <div class="i-ph:caret-up h-3 w-3" />
              </button>
            </PopoverTrigger>
            <PopoverContent
              side="top"
