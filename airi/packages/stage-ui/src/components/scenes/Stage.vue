...
import { createBatchTtsSession, createStageTtsSession } from '../../libs/speech/tts-session'
import { useAudioContext, useSpeakingStore } from '../../stores/audio'
import { useBackgroundStore } from '../../stores/background'
import { useChatOrchestratorStore } from '../../stores/chat'
import { useLlmStreamingControlStore } from '../../stores/llm-streaming-control'
import { useAiriCardStore } from '../../stores/modules'
import { useEmotionStore } from '../../stores/modules/emotion'
...
// NOTICE: browser-web-speech-api requires Google's servers and crashes in Electron with
// a network error loop that brings down the renderer. Clear any stale localStorage value
// left over from a previous session that erroneously set it as the active provider.
if (isStageTamagotchi() && activeTranscriptionProvider.value === 'browser-web-speech-api') {
...
// Maps VAD (Valence-Arousal-Dominance) scores to a Stage emotion payload.
// V: positive sentiment, A: energy/activation level, D: dominance (unused here).
function vadToEmotion(state: { v: number, a: number, d: number }): EmotionPayload {
  const { v, a } = state
  if (v > 0.3 && a > 0.3)
    return { name: Emotion.Happy, intensity: Math.min(1, (v + a) / 2) }
  if (v < -0.3 && a > 0.3)
    return { name: Emotion.Angry, intensity: Math.min(1, (-v + a) / 2) }
  if (v < -0.3 && a <= 0.3)
    return { name: Emotion.Sad, intensity: Math.min(1, -v) }
  if (Math.abs(v) <= 0.3 && a > 0.5)
    return { name: Emotion.Surprise, intensity: Math.min(1, a) }
  return { name: Emotion.Neutral, intensity: 1 }
}
...
// CPU-bound local TTS providers must not receive concurrent requests: each inference
// loads the full model into RAM and running several at once exhausts memory. This mutex
// serialises all generateSpeech calls for those providers so only one request is in-flight
// at a time. kokoro-local runs in a Web Worker (WebGPU/WASM) but is included here so it
// uses batch mode — synthesising the full LLM response in one shot instead of per-sentence
// chunks, which avoids audio gaps and ensures expressions (*...*) are handled correctly.
const LOCAL_CPU_TTS_PROVIDERS = new Set(['vieneu-tts', 'piper-tts', 'kokoro-local'])
const localCpuTtsMutex = new Mutex()
...
const speechPipeline = createSpeechPipeline<AudioBuffer>({
  tts: async (request, signal) => {
    ...
    try {
      ...
      const isLocalCpu = LOCAL_CPU_TTS_PROVIDERS.has(activeSpeechProvider.value)
      if (isLocalCpu) {
        if (signal.aborted)
          return null
        await localCpuTtsMutex.acquire()
        if (signal.aborted) {
          localCpuTtsMutex.release()
          return null
        }
      }
...
function openTtsSession(): StageTtsSession {
  // Batch mode for CPU-bound local providers: buffer the full LLM response text
  // then call TTS once rather than per-sentence. Piper/VieNeu are lightweight
  // enough that synthesising a short reply in one shot has no downsides.
  if (LOCAL_CPU_TTS_PROVIDERS.has(activeSpeechProvider.value)) {
    const batchIntentId = `batch-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`
    return createBatchTtsSession<AudioBuffer>({
      intentId: batchIntentId,
      ownerId: activeCardId.value,
      playbackManager,
      onImmediateSpecial: playSpecialToken,
      hooks: {
        onError: (err) => {
          console.error('[Speech Pipeline] batch TTS failed', {
            provider: activeSpeechProvider.value,
      ...
      tts: async (rawText, signal) => {
        if (!audioContext)
          return null

        // Strip trailing lines that are bare emotion keywords — the model sometimes
        // outputs the emotion name as plain text instead of a <|ACT|> token.
        const emotionNames = new Set(Object.values(Emotion))
        const lines = rawText.split('\n')
        while (lines.length > 0 && emotionNames.has(lines[lines.length - 1].trim().toLowerCase() as Emotion))
          lines.pop()
...
chatHookCleanups.push(onAssistantResponseEnd(async (_message) => {
  ...
  // Drive character emotion from VAD analysis when the local Python server is running
  if (_message) {
    const vadState = await emotionStore.analyzeText(_message)
    if (vadState) {
      emotionsQueue.enqueue(vadToEmotion(vadState))
    }
  }
}))
...
// Only activates when no speech provider has been explicitly chosen yet
// so it never overwrites the user's persisted settings.
let vieneuTtsAutoConfigured = false
watch(componentState, async (state) => {
  if (state !== 'mounted' || vieneuTtsAutoConfigured)
    return

  // Skip if the user has already configured a speech provider
  if (activeSpeechProvider.value && activeSpeechProvider.value !== 'speech-noop')
    return

  // Give Live2D / VRM a moment to finish its own init before opening new network requests
  await sleep(2000)
  ...
  try {
    ...
    if (res.ok) {
      vieneuTtsAutoConfigured = true
      const data = await res.json() as { voices?: string[] }
      const firstVoice = data.voices?.[0]
      providersStore.markProviderAdded('vieneu-tts')
      activeSpeechProvider.value = 'vieneu-tts'
      // NOTICE: Pinia auto-unwraps refs returned from defineStore, so access without .value
      // Model must be set — the TTS pipeline returns null when activeSpeechModel is empty
      speechStore.activeSpeechModel = 'vieneu'
      if (firstVoice)
        speechStore.activeSpeechVoiceId = firstVoice
    }
  }
  catch {
    // Python server not running — silently skip auto-config
  }
})

// Auto-configure local Whisper transcription server (whisper_server.py at port 9000).
// Probes once after mount. Only activates when no transcription provider is set yet
// so it never overwrites a provider the user has already chosen manually.
let whisperAutoConfigured = false
watch(componentState, async (state) => {
  if (state !== 'mounted' || whisperAutoConfigured)
    return
  ...
  try {
    ...
    if (res.ok) {
      ...
      activeTranscriptionProvider.value = 'openai-compatible-audio-transcription'
      activeTranscriptionModel.value = 'whisper-1'
    }
  }
  catch {
    // Whisper server not running — silently skip auto-config
  }
})
