...
// autoSend is persisted in hearing store so it survives dialog close/reopen.
// If parent explicitly binds v-model:autoSend (legacy), prefer that; otherwise
// fall back to the store so stage-tamagotchi (which doesn't bind the model) works too.
const modelAutoSend = defineModel<boolean>('autoSend')
const hearingStore = useHearingStore()
const autoSendValue = computed({
  get: () => modelAutoSend.value ?? hearingStore.autoSendEnabled,
  set: (v) => {
    hearingStore.autoSendEnabled = v
    if (modelAutoSend.value !== undefined)
      modelAutoSend.value = v
  },
})
...
<template>
  <div class="space-y-2">
    ...
    <div class="flex flex-wrap gap-2">
      ...
      <Button
        label="Auto send"
        :variant="autoSendValue ? 'primary' : 'secondary'"
        flex-1
        @click="autoSendValue = !autoSendValue"
      />
    </div>
