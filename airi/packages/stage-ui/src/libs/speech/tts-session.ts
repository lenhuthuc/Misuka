 ...
 */
export interface BatchTtsSessionOptions<TAudio = AudioBuffer> {
  intentId: string
  /** Called once with the full buffered text when finishInput() fires. */
  tts: (text: string, signal: AbortSignal) => Promise<TAudio | null>
  playbackManager: PlaybackManagerSubset<TAudio>
  ownerId?: string
  /** Called immediately for each special token (emotion / delay). */
  onImmediateSpecial?: (special: string) => void
  hooks?: StreamingSessionHooks
}

/**
 * Batch adapter: buffers all appendText() calls, then generates TTS for the
 * complete text in one shot when finishInput() fires. Suitable for CPU-bound
 * local providers (piper-tts, vieneu-tts) where the full synthesis of a
 * short response is fast enough that per-segment latency offers no benefit.
 */
export function createBatchTtsSession<TAudio = AudioBuffer>(
  options: BatchTtsSessionOptions<TAudio>,
): StageTtsSession {
  const { intentId, tts, playbackManager, hooks } = options
  let buffer = ''
  let cancelled = false
  ...
  return {
    ...
    appendSpecial(special) {
      options.onImmediateSpecial?.(special)
    },
    finishInput() {
      ...
      void (async () => {
        try {
          const audio = await tts(text, abortController.signal)
          if (cancelled || abortController.signal.aborted || !audio)
            return
