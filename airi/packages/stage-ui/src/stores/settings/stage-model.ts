...
export const useSettingsStageModel = defineStore('settings-stage-model', () => {
  ...
  async function updateStageModel() {
    ...
    if (!model && selectedModelId !== 'preset-live2d-1') {
      // NOTICE: Selected model no longer exists (e.g. IndexedDB/OPFS cleared after import).
      // Fall back to the default preset so the stage is never left blank.
      stageModelSelectedState.value = 'preset-live2d-1'
      model = await displayModelsStore.getDisplayModel('preset-live2d-1')
      if (requestId !== stageModelUpdateSequence)
        return
    }
