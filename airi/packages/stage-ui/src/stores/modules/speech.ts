...
export const useSpeechStore = defineStore('speech', () => {
  ...
  function ensureActiveSpeechModel() {
    ...
    if (!provider || provider === 'speech-noop' || provider === OFFICIAL_SPEECH_STREAMING_PROVIDER_ID)
      return
    ...
    // For the official provider, also clear the voice so auto-pick re-fires.
    // For local providers (vieneu-tts, etc.) keep the persisted voice selection.
