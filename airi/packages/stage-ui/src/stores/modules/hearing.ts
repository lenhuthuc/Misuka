  ...
  async function transcribeForRecording(recording: Blob | null | undefined) {
    ...
    try {
      if (recording && recording.size > 0) {
        ...
        const model = activeTranscriptionModel.value
        // Derive extension from the blob MIME type so the server's temp file
        // has the right suffix for ffmpeg to identify the container correctly.
        // MediaRecorder in Edge/Chrome typically produces audio/webm.
        const mime = recording.type || 'audio/webm'
        const ext = mime.includes('ogg') ? '.ogg' : mime.includes('mp4') ? '.mp4' : mime.includes('webm') ? '.webm' : '.wav'
        const result = await hearingStore.transcription(
          providerId,
          provider,
          model,
          new File([recording], `recording${ext}`),
        )
