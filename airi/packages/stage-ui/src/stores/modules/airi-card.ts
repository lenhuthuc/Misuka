...
export const useAiriCardStore = defineStore('airi-card', () => {
  ...
  return {
    ...
    systemPrompt: computed(() => {
      ...
      if (activeSpeechProvider.value === 'piper-tts') {
        const { v, a, d } = emotionStore.emotion

        components.push(
          `<user_emotion>V=${v} A=${a} D=${d} [-1,1]
V=valence(negative↔positive) A=arousal(calm↔agitated) D=dominance(submissive↔confident)
Silently adjust tone, word choice, and energy to match. Never mention VAD or simulate emotions explicitly.</user_emotion>`,
        )
      }
