...
const { configuredTranscriptionProvidersMetadata, allAudioTranscriptionProvidersMetadata: availableTranscriptionProvidersMetadata, configuredProviders } = storeToRefs(providersStore)
...
const providerSelectionProviders = computed(() => {
  ...
  return availableTranscriptionProvidersMetadata.value.filter(metadata => metadata.requiresCredentials === false || configuredProviders.value[metadata.id])
})

const showAutoAvailableProviderHint = computed(() => configuredTranscriptionProvidersMetadata.value.length === 0 && providerSelectionProviders.value.length > 0)
const activeTranscriptionProviderName = computed(() => {
  const provider = providerSelectionProviders.value.find(metadata => metadata.id === activeTranscriptionProvider.value)
  return provider?.localizedName || activeTranscriptionProvider.value
})
    ...
    <div bg="neutral-100 dark:[rgba(0,0,0,0.3)]" rounded-xl p-4 flex="~ col gap-4" class="h-fit w-full md:w-[40%]">
      <div flex="~ col gap-4">
        ...
        <div flex="~ col gap-4">
          ...
          -->
            <fieldset
              v-if="providerSelectionProviders.length > 0"
              flex="~ row gap-4"
              min-w-0 of-x-auto scroll-smooth
              role="radiogroup"
            >
              <RadioCardSimple
                v-for="metadata in providerSelectionProviders"
                :id="metadata.id"
                :key="metadata.id"
            ...
            <div v-if="showAutoAvailableProviderHint" class="mt-4 rounded-lg border border-blue-200 bg-blue-50 p-4 text-sm text-blue-700 dark:border-blue-800 dark:bg-blue-900/20 dark:text-blue-300">
              Available transcription providers can be selected without explicit setup. If you want browser-native STT, choose Web Speech API above.
            </div>
            <div v-else>
              <RouterLink
        ...
        <div v-else class="flex flex-col gap-4">
          ...
          <div class="space-y-3">
            ...
            <div v-if="activeTranscriptionProvider" class="text-xs text-neutral-500 dark:text-neutral-400">
              <div>Provider: <span class="font-medium">{{ activeTranscriptionProviderName }}</span></div>
