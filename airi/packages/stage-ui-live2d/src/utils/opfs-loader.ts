...
export class OPFSCache {
  ...
  static checkMiddleware: Middleware<OPFSContext> = async (context, next) => {
    ...
    // NOTICE: Do not pre-fetch the zip here. Fetching a large zip during app
    // startup races with Vite module loads on the HTTP/1.1 connection pool
    // (6 connections max to localhost), causing ERR_FAILED 200 (OK) mid-body.
    // ZipLoader is already designed to download zips via XHR; let it own the
    // download. saveMiddleware will cache the extracted files after ZipLoader
    // succeeds, so subsequent loads hit OPFS cache without any network request.
    context.source = blobUrl
