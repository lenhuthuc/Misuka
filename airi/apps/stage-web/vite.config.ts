...
export default defineConfig({
  ...
  plugins: [
    // Vite's @fs streaming handler can fail mid-body on Windows for large files
    // (net::ERR_FAILED 200 OK). This middleware intercepts @fs zip requests,
    // reads the entire file async, then sends it in one shot — no streaming.
    {
      name: 'serve-zip-assets',
      enforce: 'pre' as const,
      configureServer(server) {
        server.middlewares.use((req, res, next) => {
          if (!req.url?.includes('/@fs/') || !req.url.includes('.zip'))
            return next()
