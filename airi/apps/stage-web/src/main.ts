...
if (import.meta.env.DEV && !import.meta.env.SSR && 'serviceWorker' in navigator) {
  // A stale SW (e.g. from vite build/preview) can serve the cached production
  // HTML on reload, which re-registers the SW, which then intercepts fetches
  // (e.g. Live2D zip → ERR_FAILED). We must:
  //   1. unregister the SW
  //   2. delete all SW caches (so the reload can't be served from cache)
  //   3. then reload — the fresh page load goes straight to the Vite dev server
