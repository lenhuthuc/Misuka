...
import { createEmotionVadRoutes } from './routes/emotion-vad'
...
export async function buildApp(deps: AppDeps) {
  ...
  // NOTICE: Registered before the global bodyLimit(1 MB) so the per-route
  // bodyLimit(10 MB) inside the handler wins for large audio uploads.
  app.route('/api/v1/audio/emotion-vad', createEmotionVadRoutes({
    env: deps.env,
    auth: deps.auth,
  }))
