...
import { useEmotionVAD } from '../composables/useEmotionVAD'
...
const emotionVAD = useEmotionVAD()
...
function buildSystemMessage(): string {
  const caps = `You are a helpful assistant. You will gain powers from the following capabilities:\n\n## Capabilities\n\n${capabilities.map(cap => `### ${cap.name}\n\n${cap.description}`).join('\n')}\n\n`
  const vadContext = emotionVAD.getSystemPromptContext()
  return vadContext ? `${caps}\n${vadContext}\n` : caps
}
...
async function handleChatSendMessage() {
  ...
  // Refresh system message with the latest fused VAD (updated by analyzeAudio
  // which the caller triggers after each speech-ready event from the VAD worklet)
  const userText = sendingMessage.value
  messages.value[0] = { role: 'system', content: buildSystemMessage() }
      ...
      <div flex flex-col gap-2>
        ...
        <div grid grid-cols-2 gap-2>
          ...
          <label flex items-center gap-2>
            ...
            <Input v-model="emotionVAD.serverBaseUrl.value" placeholder="http://localhost:3000" />
          </label>
        </div>
        <!-- Emotion VAD status -->
        <div v-if="emotionVAD.fusedVAD.value" flex items-center gap-3 text-xs text-neutral-500 dark:text-neutral-400>
          <span>VAD</span>
          <span>V {{ emotionVAD.fusedVAD.value.valence >= 0 ? '+' : '' }}{{ emotionVAD.fusedVAD.value.valence.toFixed(2) }}</span>
          <span>A {{ emotionVAD.fusedVAD.value.arousal >= 0 ? '+' : '' }}{{ emotionVAD.fusedVAD.value.arousal.toFixed(2) }}</span>
          <span>D {{ emotionVAD.fusedVAD.value.dominance >= 0 ? '+' : '' }}{{ emotionVAD.fusedVAD.value.dominance.toFixed(2) }}</span>
          <span v-if="emotionVAD.isAnalyzingAudio.value" i-svg-spinners:3-dots-scale />
        </div>
        <div v-if="emotionVAD.error.value" text-xs text-red-500>
          {{ emotionVAD.error.value }}
        </div>
      </div>
      <div v-if="false">
